          ...
          <td colspan="4">
            <div class="price-wrap mt20 mb20">
              <div>
                ...
                <p>
                  <span>{{ __localeString(data.buyAdvertisingProductVideoCostPrice) }}</span> {{ $t("common-unit-price") }}
                </p>
              </div>
              <div v-if="data.buyAdvertisingProductVideoPriceRate > 0"><img src="@/assets/images/minus.png" alt="" /></div>
              <div v-if="data.buyAdvertisingProductVideoPriceRate > 0">
                <p class="subject">
                  {{ $t("pay-price-discount") }}<span class="discount">{{ data.buyAdvertisingProductVideoPriceRate }}%</span>
                </p>
                <p>
                  <span>{{ __localeString(data.buyAdvertisingProductVideoDiscountPrice) }}</span> {{ $t("common-unit-price") }}
                </p>
              </div>
  ...
  & tr {
    & td {
      ...
      & .header {
        ...
        & .before {
          @include NotoSans(1.9, 500, #828282);
        }
        & .current {
          @include NotoSans(1.9, 500, #ff0000);
        }
        & .after {
          @include NotoSans(1.9, 500, #0047ff);
        }
        & .cancel {
          @include NotoSans(1.9, 500, #bdbdbd);
        }
        & .name {
          @include NotoSans(2.6, 500, #4f4f4f);
        }
        & .btn {
